...
> 存放原始源稿与计划格式变体。2026-09-08 整理：下方矩阵保留早期 RAG 设计用途；当前一期只按已确认格式选样本，转换/保存判定见[提测草案](../../docs/10-知识库一期提测用例草案.md)。`manifest-v1.yaml` 的 ready 和 expected_format 不代表产品测试通过或格式承诺。
...
实际有 10 份 Markdown 源稿（含合并版说明和独立新旧版本），以及 1 份文本异常样本。它们是事实和结构的参考，按一期已确认格式制作必要变体；不默认追加 PDF/XLSX/OCR。空文件和超限文件仍待按产品限制生成；现有异常 TXT 能否触发失败也需验证。

上传前先核对 `manifest-v1.yaml` 的 `file_name`、`status` 和产品实际支持格式；`status: derived` 的合并版本不要与其版本变体同时上传。一期上传后在运行目录记录 SHA-256、转换后正文、修改保存后读取结果和必要的内容位置证据；Chunk 相关记录留到 RAG 阶段。
...
1. 确认一期实际支持格式、默认归属和权限。
2. 选少量现有源稿，核对文字、数字、表格与标题。
3. 按已确认范围补图片、真实 Word 结构、编码或异常变体，记录文件与哈希。
4. 对每份样本写清预期保留内容，执行上传、修改保存和找回。
5. 将真实结果留在运行目录，不能用源稿存在代替产品通过。
